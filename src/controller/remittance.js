const Remittance = require('../model/remittance');
...
exports.createRemittance = async (req, res) => {
  try {
    ...
    const remittance = new Remittance({ city, optionBestDescribeYou, name, mobile, email, monthlyImport_Export, purpose });
    // Save the remittance document to the database
    const result = await remittance.save();
    return res.status(201).json({ message: 'Remittance created successfully', data: result });
  } catch (error) {
...
exports.getAllRemittances = async (req, res) => {
  try {
    ...
    return res.status(200).json({ data: remittances });
  } catch (error) {
    console.error(error); // Log the error for debugging purposes
    return res.status(500).json({ message: 'Internal server error' });
  }
...
exports.updateRemittanceById = async (req, res) => {
  try {
    ...
    if (result) {
      return res.status(200).json({ message: 'Remittance updated successfully', data: result });
    } else {
...
exports.deleteRemittanceById = async (req, res) => {
  try {
    ...
    if (result) {
      return res.status(200).json({ message: 'Remittance deleted successfully' });
    } else {
      return res.status(404).json({ message: 'Remittance not found' });
    }
  } catch (error) {

...
exports.createPrepaidTravel_unload = async (req, res) => {
  try {
    let data = { selectCity: req.body.selectCity, currency: req.body.currency, forexAmount: req.body.forexAmount, buy_reload_unload: "unload" };
...
exports.updatePrepaidTravelById_unload = async (req, res) => {
  try {
    ...
    const upda = await prepaidtravelModel.findByIdAndUpdate({ _id: req.params.id }, { $set: { uploadPanCard: req.body.uploadPanCard, PassportFront: req.body.PassportFront, PassportBack: req.body.PassportBack, }, }, { new: true });
    if (!upda) {
...
exports.updatePrepaidAccountDetails_unload = async (req, res) => {
  try {
    ...
    const data = { data1: req.body.data1, data2: req.body.data2, exchangeRate: exchangeRate, transferAmountInFCY: req.body.transferAmountInFCY1, Total: prepaidtravel.total, RemittanceServiceCharge: req.body.RemittanceServiceCharge, GstOnCharge: req.body.GstOnCharge, GstOnCurrencyConversion: req.body.GstOnCurrencyConversion, };
    const TotalFundingAmtInINR = prepaidtravel.total + parseInt(data.transferAmountInFCY) + parseInt(data.RemittanceServiceCharge) + parseInt(data.GstOnCharge) + parseInt(data.GstOnCurrencyConversion)
    const TotalOfAllChargesAndTaxes = parseInt(data.transferAmountInFCY) + parseInt(data.RemittanceServiceCharge) + parseInt(data.GstOnCharge) + parseInt(data.GstOnCurrencyConversion)
    const currency = await prepaidtravelModel.findByIdAndUpdate({ _id: prepaidtravel._id }, { $set: { data1: data.data1, data2: data.data2, exchangeRate: data.exchangeRate, transferAmountInFCY: data.transferAmountInFCY, Total: data.Total, RemittanceServiceCharge: data.RemittanceServiceCharge, GstOnCharge: data.GstOnCharge, GstOnCurrencyConversion: data.GstOnCurrencyConversion, TotalFundingAmtInINR: TotalFundingAmtInINR, TotalOfAllChargesAndTaxes: TotalOfAllChargesAndTaxes, }, }, { new: true });
